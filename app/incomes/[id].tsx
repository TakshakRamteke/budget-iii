...
export default function IncomePage() {
    ...
    useEffect(() => {
        ...
        db.transaction((tx) =>
            tx.executeSql(
                `SELECT incomes.*, incomesCategouries.name AS category FROM incomes LEFT JOIN incomesCategouries ON incomes.categoryId=incomesCategouries.id WHERE incomes.id=${id}`,
                [],
                (_, { rows }) => {
                    setIncome(rows._array[0]);
